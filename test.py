...
def test(model, model_path,test_loader):
    ...
    with torch.no_grad():
        for batch_idx, (data, target) in enumerate(test_loader):
            ...
            output,mid_vector = model(data)
            #print(output)
            prob=output[:,0].cpu().numpy() 
            #print(prob)
            if torch.equal(output.ge(0.5).float(),target.float()):
                print(batch_idx)
                print(output.ge(0.5).float())
                print(target.float())
    ...
    return predicates_all, target_all,prob,mid_vector,output
...
if __name__ == '__main__':
    ...
    model_path = './best_model/COVID19_dense169_85_85.path.tar'
    predicates_all, target_all ,prob,mid_vector,output= test(cnn, model_path, test_loader)
    ...
    print("AUC",roc_auc_score(target_all, prob))
    print(confusion_matrix(target_all, predicates_all))
    
    target_all,prob=np.array(target_all),np.array(prob)
    prob_target=np.vstack((prob,target_all))
    print(prob_target)
    def save_r(result,fname):
        with open(fname,'wb') as fo:
            pickle.dump(result,fo)
        
    save_r(prob_target,'dense169_new.pkl')
    mid_vector=mid_vector.cpu().numpy()
    print(mid_vector.shape)
    save_r(mid_vector,'dense169_mid_vector.pkl')
    
    output=output.cpu().numpy()
    print(output.shape)
    save_r(output,'dense169_output.pkl')
    
    
    predicates_all=np.array(predicates_all)
    print(predicates_all.shape)
    save_r(predicates_all,'dense169_predicates_all.pkl')
